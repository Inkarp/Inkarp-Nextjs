...
function StepHexagon({ number }) {
  return (
    <span
      className="relative flex size-14 shrink-0 items-center justify-center bg-white font-maxot text-base font-bold text-[#BE0010] shadow-sm shadow-zinc-950/5 [clip-path:polygon(25%_0%,75%_0%,100%_50%,75%_100%,25%_100%,0%_50%)]"
      aria-hidden="true"
    >
      {number}
    </span>
    ...
    <section className="relative overflow-hidden bg-[#fff3f4] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
      ...
      <div className="relative mx-auto max-w-6xl">
            ...
            <div className="relative flex flex-1 flex-col" key={step.number}>
              <div className="relative flex items-center">
                ...
                {index < steps.length - 1 ? (
                  <span
                    aria-hidden="true"
                    className="absolute left-14 top-1/2 h-px w-[calc(100%-3.5rem)] -translate-y-1/2 border-t-2 border-dashed border-zinc-300"
                  />
                ) : null}
              </div>
              ...
              <div className="mt-5 flex gap-3 pr-4">
                ...
                />
                <div>
                  <h3 className="font-maxot text-base font-bold text-zinc-950">
                    {step.title}
                  </h3>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:hidden">
          {steps.map((step) => (
            <div
              className="flex gap-3 rounded-xl border border-white/60 bg-white p-4 shadow-sm shadow-zinc-950/5"
              key={step.number}
            >
              <StepHexagon number={step.number} />
              <div>
